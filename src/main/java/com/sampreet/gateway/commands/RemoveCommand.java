...
public class RemoveCommand extends SubCommand {
    ...
    public RemoveCommand(Gateway plugin) {
        super(plugin);
    }
    ...
    public void execute(CommandSender sender, String @NotNull [] args) {
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            plugin.getServer().getScheduler().runTask(plugin, () -> handleRemove(sender, username, playerEntry));
        });
    }

    private void handleRemove(CommandSender sender, String username, PlayerEntry playerEntry) {
        if (!plugin.getJsonHelper().check(playerEntry)) {
            sendMessage(sender, "messages.commands.errors.not_whitelisted", username);
            return;
        }

        plugin.getJsonHelper().remove(playerEntry);
        plugin.getJsonHelper().save();

        sendMessage(sender, "messages.commands.remove", username);
    }
